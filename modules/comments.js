import { db, state, isAdmin, aiSuggestComment, getMediaUrl, createThinkingUI } from './config.js';
...
export async function openCmtModal(postId) {
  ...
  // Local post ekanini aniqlash
  state.cmtPostIsLocal = !!(state.allPosts?.find(p => p.id === postId)?._fromLocal);
...
export async function loadCmtModal(postId) {
  ...
  try {
    const col  = state.cmtPostIsLocal ? 'local-posts' : 'posts';
    const snap = await getDocs(
      query(collection(db, col, postId, 'comments'), orderBy('createdAt','asc'))
    );
    ...
    if (!cmts.length) {
      list.innerHTML = `<div class="cmt-empty">
        <svg class="opacity-30 mb-8px" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
    ...
    list.querySelectorAll('.cmt-del').forEach(b => b.addEventListener('click', async () => {
      ...
      b.disabled = true;
      try {
        const delCol = state.cmtPostIsLocal ? 'local-posts' : 'posts';
        await deleteDoc(doc(db, delCol, b.dataset.post, 'comments', b.dataset.cmt));
        // commentCount ni kamaytirish (atomic)
        await updateDoc(doc(db, delCol, b.dataset.post), {
          commentCount: increment(-1)
        }).catch(() => setDoc(doc(db, delCol, b.dataset.post), { commentCount: 0 }, { merge: true }));
...
export async function sendCmtModal() {
  ...
  try {
    ...
    const sendCol = state.cmtPostIsLocal ? 'local-posts' : 'posts';
    await addDoc(collection(db, sendCol, state.cmtPostId, 'comments'), {
      userId:    state.me.uid,
    ...
    await updateDoc(doc(db, sendCol, state.cmtPostId), {
      commentCount: increment(1)
    }).catch(() => setDoc(doc(db, sendCol, state.cmtPostId), { commentCount: 1 }, { merge: true }));
...
aiCmtBtn.addEventListener('click', async () => {
  ...
  try {
    let imageUrl = null, postText = '', fileName = null, mediaType = null;
    if (state.cmtPostId) {
      const col = state.cmtPostIsLocal ? 'local-posts' : 'posts';
      const snap = await getDoc(doc(db, col, state.cmtPostId));
      if (snap.exists()) {
        const d = snap.data();
    ...
    if (state.me) {
      try {
        ...
        commenterName = meSnap.data()?.fullName || state.me.displayName || null;
      } catch {}
    }
    let prevComments = [];
    if (state.cmtPostId) {
      try {
        const col2 = state.cmtPostIsLocal ? 'local-posts' : 'posts';
        const cmtSnap = await getDocs(
          query(collection(db, col2, state.cmtPostId, 'comments'), orderBy('createdAt', 'asc'))
        );
        prevComments = cmtSnap.docs.map(d => ({ userName: d.data().userName || '?', text: d.data().text || '' }));
      } catch {}
    }

    const suggestion = await aiSuggestComment(
      imageUrl, postText, fileName, mediaType, commenterName, prevComments,
