...
const _ADMIN_UID = 'cS9Riz2K4xgW1i4PVboWoQfhGok2';

/** Joriy foydalanuvchi admin ekanligini tekshirish */
export function isAdmin() {
  return !!(state.me && state.me.uid === _ADMIN_UID);
}
...
const AI_RATE_LS_KEY    = 'mrgram_ai_usage_ts';

// ── Firestore: ai_rate/{uid} → { timestamps: number[] } ─────────────────
// localStorage faqat Firestore xato bo'lganda fallback sifatida ishlatiladi.

function _lsReadTs() {
  try { return JSON.parse(localStorage.getItem(AI_RATE_LS_KEY) || '[]'); }
  catch { return []; }
}
function _lsWriteTs(arr) {
  try { localStorage.setItem(AI_RATE_LS_KEY, JSON.stringify(arr)); } catch {}
}

async function _fsReadTs() {
  const uid = state.me?.uid;
  if (!uid) return null;          // tizimga kirilmagan — LS fallback
  try {
    const snap = await getDoc(doc(db, 'ai_rate', uid));
    return snap.exists() ? (snap.data().timestamps || []) : [];
  } catch { return null; }        // xato — LS fallback
}

async function _fsWriteTs(arr) {
  const uid = state.me?.uid;
  if (!uid) return;
  try {
    await setDoc(doc(db, 'ai_rate', uid), { timestamps: arr, updatedAt: Date.now() });
  } catch {}
}

/** Hozir AI ishlatish mumkinmi? { allowed, waitSec }
 *  UI tugmalarida ham chaqiriladi — Firestore'dan o'qiydi. */
export async function aiRateLimitStatus() {
  const now  = Date.now();
  const fsTs = await _fsReadTs();
  const arr  = (fsTs !== null ? fsTs : _lsReadTs()).filter(t => now - t < AI_RATE_WINDOW_MS);
...
async function _enforceAiRateLimit() {
  const now  = Date.now();
  const fsTs = await _fsReadTs();
  const useFs = fsTs !== null;
  const arr  = (useFs ? fsTs : _lsReadTs()).filter(t => now - t < AI_RATE_WINDOW_MS);
  ...
  arr.push(now);
  if (useFs) {
    await _fsWriteTs(arr);
    _lsWriteTs(arr);              // LS ni ham yangilaymiz (oflayn konsistentlik)
  } else {
    _lsWriteTs(arr);              // Firestore xato — faqat LS
  }
}
...
export async function extractVideoWithAudio(videoUrl, apiKey, frameCount = 'auto') {
  const MAX_W = 640, MAX_H = 360, QUALITY = 0.72;
  ...
  return new Promise(resolve => {
    ...
    video.addEventListener('loadedmetadata', async () => {
      ...
      const effectiveCount = (frameCount === 'auto' || !frameCount)
        ? (dur <= 15  ? 3          // ≤15s  — qisqa klib: 3 kadr yetarli
         : dur <= 30  ? 4          // ≤30s  — standart: 4 kadr
         : dur <= 60  ? 6          // ≤60s  — o'rta: 6 kadr
         : dur <= 180 ? 8          // ≤3min — uzun: 8 kadr
         :              10)        //  >3min — juda uzun: 10 kadr (max)
        : frameCount;
...
async function _aiGenerateCaptionImpl(source, fileName, mimeType, uploaderName, onStep) {
  onStep?.('preparing');
...
- Kontent: tog'da sayohat videosi → Caption: "Bugun tog'ga chiqdik, manzarasi hayratlanarli edi"`;
  ...
  // Orqaga moslik: eski chaqiruvlar shunchaki rasm data-URL/URL berardi
    ...
    return groqRequest([
      { role: 'system', content: uploaderCtx },
      {
        ...
        content: [
          ...
          { type: 'text', text: "Ushbu rasm uchun qisqa, tabiiy o'zbek tilida caption yaz. Faqat captionni yoz, boshqa hech narsa yozma. 1-2 jumla bo'lsin." }
        ]
      }
    ], { model: VISION_MODEL });
  }

  // Video — kadr + audio segment birga
  if ((mimeType || '').startsWith('video/')) {
    ...
    if (results.length > 0) {
      onStep?.('thinking');
      const parts = buildVideoAudioContent(results, "Shu mazmun asosida qisqa, tabiiy o'zbek tilida birinchi shaxsda caption yaz. Faqat captionni yaz, 1-2 jumla.");
      return groqRequest([{ role: 'system', content: uploaderCtx }, { role: 'user', content: parts }], { model: VISION_MODEL, max_tokens: 150, temperature: 1.0 });
    }
    onStep?.('thinking');
    return groqRequest([{ role: 'system', content: uploaderCtx }, { role: 'user', content: "Video post uchun qisqa, tabiiy o'zbek tilida birinchi shaxsda caption yaz. Faqat captionni yaz, 1-2 jumla." }], { max_tokens: 150 });
  }
  ...
  if (info.kind === 'image') {
    return groqRequest([
      { role: 'system', content: uploaderCtx },
      {
        ...
        content: [
          { type: 'image_url', image_url: { url: source } },
          { type: 'text', text: "Ushbu rasm uchun qisqa, tabiiy o'zbek tilida birinchi shaxsda caption yaz. Faqat captionni yoz, boshqa hech narsa yozma. 1-2 jumla bo'lsin." }
        ]
      }
    ], { model: VISION_MODEL });
  }
...
export async function aiGenerateCaption(source, fileName, mimeType, uploaderName, onStep) {
  ...
  const result = await _aiGenerateCaptionImpl(source, fileName, mimeType, uploaderName, onStep);
...
{"flagged": true yoki false, "reason": "qisqa o'zbek tilida sabab, 1 jumla"}`;
  ...
  try {
    ...
    ], { model: judgeModel, temperature: 0, max_tokens: 200, response_format: { type: 'json_object' } });
...
/** Post fayli bo'yicha AI izoh taklifi — commenter nomidan, tabiiy ohangda */
export async function aiSuggestComment(mediaUrl, postText, fileName, mediaType, commenterName, prevComments, onStep) {
  onStep?.('preparing');
  await _enforceAiRateLimit();

  // Faqat birinchi ismni olamiz
  const commenterFirst = commenterName
    ? commenterName.trim().split(/[\s\d.]+/)[0] || commenterName
    : null;
...
Namunalar (faqat USLUB va UZUNLIK uchun — mazmunini ko'chirma, har doim pastda berilgan HAQIQIY post asosida yoz):
- Kontent: kechki ovqat fotosi → Izoh: "Voy zo'r ekan, menga ham chaqirardingda 😄"
- Kontent: yangi mashina sotib olgani haqida post → Izoh: "Tabriklayman, juda chiroyli rang tanlagansan!"`;

  // Commenter konteksti
  const commenterCtx = (commenterFirst
    ? `Sen "${commenterFirst}" ismli foydalanuvchisan va do'sting postiga izoh yozyapsan. Izohni birinchi shaxsda yoz — xuddi o'sha odam yozayotgandek. Tabiiy, samimiy, qisqa. Rasmiy iboralar ishlatma. TIL QOIDASI: Javobingni FAQAT O'ZBEK TILIDA yoz. Post yoki rasm inglizcha, ruscha yoki boshqa tilda bo'lsa ham — sen o'zbek tilida javob ber. Chet tildagi narsa/nom/iqtibosni o'zbek tiliga tarjima qilib tushuntir.`
    : `Sen ijtimoiy tarmoq foydalanuvchisisisan va birovning postiga samimiy izoh yozyapsan. Tabiiy va qisqa yoz. TIL QOIDASI: FAQAT O'ZBEK TILIDA yoz — post boshqa tilda bo'lsa ham.`)
    + COMMENT_FEWSHOT;
  ...
  if ((mediaType || '').startsWith('video/')) {
    ...
    if (results.length > 0) {
      onStep?.('thinking');
      const instr = `${textPart}${cmtCtx}\n${toneGuide}\nVideo kadrlarda nima ko'rayotganingga qarab, samimiy o'zbek tilida 1 jumla izoh yaz. Faqat izohni yaz.`;
      const parts = buildVideoAudioContent(results, instr);
      return _safe(await groqRequest([{ role: 'system', content: commenterCtx }, { role: 'user', content: parts }], { model: VISION_MODEL, max_tokens: 120, temperature: 0.85 }));
    }
    onStep?.('thinking');
    return _safe(await groqRequest([
      { role: 'system', content: commenterCtx },
...
async function _parseAboutReply(raw) {
  try {
    ...
    console.warn('⚠️ AI fikri JSON parse xatosi:', err.message);
  }
 ...
 */
export async function aiAboutPost({ imageUrl, mediaUrl, fileName, mediaType, text, posterName, likes, views, commentCount, createdAt, prevComments, onStep } = {}) {
  onStep?.('preparing');
  await _enforceAiRateLimit();
...
{"title": "GTA V dunyosi", "comment": "Bu sahna GTA V o'yinining ochiq dunyo missiyalaridan biriga o'xshaydi, grafikasi va tafsilotlari ajoyib chiqgan.", "mood": "serious"}`;
  ...
  if (!url) {
    ...
    ], { temperature: 1.15, max_tokens: 320, response_format: { type: 'json_object' } });
  ...
  if ((mediaType || '').startsWith('video/')) {
    ...
    ], { temperature: 0.9, max_tokens: 320, response_format: { type: 'json_object' } });
  ...
  ], { model: info.kind === 'image' ? VISION_MODEL : TEXT_MODEL, temperature: 1.15, max_tokens: 320, response_format: { type: 'json_object' } });
  return await _parseAboutReply(raw);
}
