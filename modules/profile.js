...
async function _paintProfile(ud) {
  ...
  if (metaEl) {
    let metaHtml = '';
    if (ud.location) metaHtml += `<span class="profile-meta-item"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>${ud.location}</span>`;
  ...
  $('profileAvi').onclick = () => {
    ...
    if (zm) {
      ...
      if (!editBtn) {
        ...
        editBtn.innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg> Rasmni tahrirlash`;
...
export async function renderProfileGrid(posts) {
  if (!posts.length) {
    $('profileGrid').innerHTML = `<div class="empty">
      <svg class="opacity-30 mx-auto mb-10px d-block" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.3">
...
export async function renderUserProfileModal(uid) {
  ...
  $('upBody').innerHTML = `
    ...
    <div class="up-info">
      ...
      <button class="up-follow-btn ${isF?'is-following':'not-following'}" id="upFollowBtn" data-uid="${uid}"><svg class="follow-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></button>
  ...
  if (followBtn) {
    followBtn.onclick = async () => {
      ...
      if (currently) {
        ...
        if (feedEl) {
          const svgPlus = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>`;
