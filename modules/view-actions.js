...
async function _initAiModeration() {
  ...
  section.innerHTML = `<div class="aimod-wrap" id="aimodWrap"><div class="aimod-empty">Yuklanmoqda…</div></div>`;
  ...
  _aiTasksUnsub = onSnapshot(q, async (snap) => {
    for (const taskDoc of snap.docs) {
      ...
      try {
        ...
        await updateDoc(doc(db, 'adminTasks', taskDoc.id), {
          done:       true,
...
`;
  ...
  _noticeUnsub = onSnapshot(doc(db, 'adminNotice', 'global'), snap => {
    if (snap.exists()) {
      ...
      currentCard.innerHTML = `
        ...
        <div class="bc-current-target"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m3 11 18-5v12L3 14v-3z"/><path d="M11.6 16.8a3 3 0 1 1-5.8-1.6"/></svg> ${TARGET_LABELS[d.target] || d.target}</div>
      `;
    } else {
  ...
  sendBtn.addEventListener('click', async () => {
    ...
    try {
      await setDoc(doc(db, 'adminNotice', 'global'), {
        text: body,
        target,
  ...
  delBtn.addEventListener('click', async () => {
    ...
    try {
      await deleteDoc(doc(db, 'adminNotice', 'global'));
